...
class GenBook:
    def __init__(self, user_id, store_id):
        ...
        # Allow both 200 (new store) and 513/514 (store already exists) for resilience
        if code not in [200, 513, 514]:
            assert code == 200, f"Failed to create store: code={code}"
    ...
    def gen(
        ...
        for bk in books:
            ...
            # Allow both 200 (new book) and 516 (book already exists) for resilience
            if code not in [200, 516]:
                assert code == 200, f"Failed to add book: code={code}"
